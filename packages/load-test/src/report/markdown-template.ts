...
export function generateMarkdownReport(report: TestReport): string {
  ...
  lines.push(
    `**Status:** ${report.summary.successRate >= 95 ? "✅ PASS" : "⚠️ WARNING"}`
  );
  lines.push("");
  ...
  const maxEventLoopLag =
    report.systemSnapshots.length > 0
  ...
  const avgCpu =
    report.systemSnapshots.length > 0
      ? report.systemSnapshots.reduce((sum, s) => sum + s.cpu, 0) /
        report.systemSnapshots.length
      : 0;

  lines.push("| Metric | Value | Status |");
  lines.push("|--------|-------|--------|");
  lines.push(
    `| Event Loop Lag (max) | ${maxEventLoopLag.toFixed(2)}ms | ${maxEventLoopLag < 10 ? "✅ Good" : maxEventLoopLag < 50 ? "⚠️ Elevated" : "❌ Critical"} |`
  );
  lines.push(
    `| API Response Time (avg) | ${avgApiResponse.toFixed(0)}ms | ${avgApiResponse < 200 ? "✅ Fast" : avgApiResponse < 500 ? "⚠️ Moderate" : "❌ Slow"} |`
  );
  lines.push(
    `| Queue Depth (max) | ${maxQueueDepth} | ${maxQueueDepth < 10 ? "✅ Normal" : maxQueueDepth < 30 ? "⚠️ Backlogged" : "❌ Overloaded"} |`
  );
  lines.push(
    `| CPU Usage (avg) | ${avgCpu.toFixed(1)}% | ${avgCpu < 70 ? "✅ Normal" : avgCpu < 85 ? "⚠️ High" : "❌ Critical"} |`
  );
  lines.push("");
  ...
  if (avgApiResponse < 200) {
    ...
    lines.push(
      "- ⚠️ **Slow Response (500-1000ms)**: Users experience noticeable delay. May feel sluggish."
    );
  } else {
    lines.push(
      "- ❌ **Very Slow Response (> 1000ms)**: Users experience significant delay. Poor user experience."
    );
  }
  lines.push(
    `- **P95 Response Time**: ${report.summary.p95.toFixed(0)}ms - ${report.summary.p95 < 500 ? "95% of users experience fast responses" : report.summary.p95 < 1000 ? "95% of users experience acceptable delays" : "95% of users experience slow responses"}`
  );
  lines.push("");
  ...
  if (maxEventLoopLag < 10) {
    ...
    lines.push(
      "- ❌ **Critical (> 100ms)**: Severe blocking. Users experience significant delays and potential timeouts."
    );
  }
  lines.push(
    `- **Max Event Loop Lag**: ${maxEventLoopLag.toFixed(2)}ms during test`
  );
  lines.push("");
  ...
  if (maxQueueDepth < 10) {
    ...
    lines.push(
      "- ⚠️ **Heavy Backlog (30-50 jobs)**: Significant delay. Users experience long wait times."
    );
  } else {
    lines.push(
  ...
  lines.push("");
  lines.push(
    `Based on this test configuration (${report.config.concurrency} concurrent requests):`
  );
  lines.push(
    `- **Test Load**: Simulated ${report.config.concurrency} concurrent users`
  );
  lines.push(
    `- **Success Rate**: ${report.summary.successRate.toFixed(1)}% - ${report.summary.successRate >= 99 ? "Excellent reliability" : report.summary.successRate >= 95 ? "Good reliability" : "Needs improvement"}`
  );
  if (uxScore >= 80) {
    lines.push(
      `- **Capacity Assessment**: ✅ System can handle ${report.config.concurrency}+ concurrent users with good performance`
    );
  } else if (uxScore >= 60) {
    lines.push(
      `- **Capacity Assessment**: ⚠️ System handles ${report.config.concurrency} concurrent users but performance is degraded`
    );
  } else {
    lines.push(
      `- **Capacity Assessment**: ❌ System struggles with ${report.config.concurrency} concurrent users. Consider optimization or scaling.`
    );
  }
  lines.push("");
  ...
  lines.push(
    `| Total Requests | ${report.config.totalRequests || "All files"} |`
  );
  lines.push(
    `| Test Duration | ${report.config.duration ? `${report.config.duration}s` : "N/A"} |`
  );
  lines.push(`| Randomize | ${report.config.randomize ? "Yes" : "No"} |`);
  lines.push(
    `| Delay Range | ${report.config.delayRange ? `${report.config.delayRange[0]}-${report.config.delayRange[1]}ms` : "N/A"} |`
  );
  lines.push(
    `| Include Health Check | ${report.config.includeHealthCheck ? "Yes" : "No"} |`
  );
  ...
  lines.push(
    `| Successful | ${report.summary.successful.toLocaleString()} (${report.summary.successRate.toFixed(2)}%) |`
  );
  lines.push(
    `| Failed | ${report.summary.failed.toLocaleString()} (${(100 - report.summary.successRate).toFixed(2)}%) |`
  );
  lines.push(
    `| Min Response Time | ${report.summary.minResponseTime.toFixed(0)}ms |`
  );
  lines.push(
    `| Max Response Time | ${report.summary.maxResponseTime.toFixed(0)}ms |`
  );
  lines.push(
  ...
  lines.push(
    `| Requests/Second (avg) | ${report.summary.requestsPerSecond.toFixed(2)} |`
  );
  lines.push("");
  ...
  if (report.systemSnapshots.length > 0) {
    ...
    const memValues = report.systemSnapshots.map(
      (s) => s.memory.used / 1024 / 1024
    ); // MB
    ...
    lines.push("|--------|-------|------|-----|--------|");
    lines.push(
      `| Heap Used | ${memStart.toFixed(0)} MB | ${memPeak.toFixed(0)} MB | ${memEnd.toFixed(0)} MB | ${(memEnd - memStart).toFixed(0)} MB |`
    );
    lines.push("");
  ...
  if (report.systemSnapshots.length > 0) {
    ...
    const completedValues = report.systemSnapshots.map(
      (s) => s.queue.completed
    );
    const timestamps = report.systemSnapshots.map((s) => s.timestamp);
